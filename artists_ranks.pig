...
token_tags = FOREACH songs_with_playcount GENERATE raw_musicinfo::track_id, raw_musicinfo::artist, TOKENIZE(raw_musicinfo::tags, ', ') AS genre, playcount;
...
ranked_artists_tags = FOREACH groupagain {
    ordergenre = ORDER $1 BY count DESC;
    GENERATE FLATTEN(Enumerate(ordergenre));
};
